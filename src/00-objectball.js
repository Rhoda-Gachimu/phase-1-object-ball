const gameObject = function () {
    ...
    home: {
        ...
        players:{
            ...
            'Reggies Evans': {number:30, shoe:14, points:12, rebounds:12, assists:12, steals:12, blocks:12, slamDunks:7},
...
function numPointsScored(playerName){
    ...
    for (let gameKey in game) {
        stats = game[gameKey].players[playerName]
    }
    return stats.points
}
// Return the shoe size for a specific player
...
// Return an array of team colors when passed the team name
function teamColors(teamName) {
    const game = gameObject()
    let colors
    for (let gameKey in game) {
        if (game[gameKey].teamName === teamName) {
            colors = game[gameKey].colors
        }
    }
    return colors
}
